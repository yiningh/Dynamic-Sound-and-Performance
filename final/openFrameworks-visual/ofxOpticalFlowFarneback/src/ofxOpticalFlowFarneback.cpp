...
void ofxOpticalFlowLK::draw(int width, int height,  float lineScale, int res, int mode ) {
	bool rightSize = (sizeSml.width == width && sizeSml.height == height);
	...
	for(int x=0; x<width; x+=res) {
		for(int y=0; y<height; y+=res) {
            ...
            if( mode == 0 ){
                ...
                float noise = (cos(ofGetFrameNum()*0.5), sin(ofGetFrameNum()*0.5));
                ofSetColor(255,255,255,255*0.3);
                ofCircle(x,y, vel.length()*1.5 );
